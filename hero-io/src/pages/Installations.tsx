...
function sortReducer(state: Partial<AppData>[], action: SortAction) {
  switch (action.type) {
    case "ASC":
...
function Installations({}: Props) {
  ...
  const handleClick = (item: Partial<AppData>) => {
    removeItem(item);
  };
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    dispatch({ type: e.target.value as "ASC" | "DESC" });
  };
  const sortedData = sort.length ? sort : data;
  return (
    <>
      <section className="flex-centered-y gap-4">
